...
for run in range (1,runs+1):
 ...
 #adding the data for the plots
        ...
        #t_line.append(t)
    maximum_inf = inf_line[0]
    for i in range (1,100):
        if inf_line[i] > maximum_inf:
            maximum_inf = inf_line[i]
    trial_peak_inf.append(maximum_inf)
    trial_num_of_fatalities.append(num_of_fatalities)
    trials.append(run)
    print('Run: ' + str(run))
...
workbook = xlsxwriter.Workbook('data_from_model_2.xlsx')
worksheet=workbook.add_worksheet()

for i in range(0,runs):
    worksheet.write('A' + str(i+1),trial_num_of_fatalities[i])
    worksheet.write('B' + str(i+1),trial_peak_inf[i])
    print(i)
workbook.close()
    
plt.plot(trials,trial_num_of_fatalities,'r-')
plt.show()
